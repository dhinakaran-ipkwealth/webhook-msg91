...
async function listSenderReportRowsForCustomReport(filters = {}) {
  await requireMongoDb();
  const query = {};
  addDateFieldConditions(query, ["sentAt", "requestedAt"], filters);
...
async function getInboundReplyMapForUpload(uploadId) {
  ...
  // byNumberId : Map<numberId, event[]>
  //   Events where matchedNumberId was written back by applyInboundReplyToReports.
  //   These are the only events we can reliably attribute to a specific sent row.
  //
  // byMobile is intentionally NOT populated.  Using all-events-for-a-mobile caused
  // unrelated replies (testing, old campaigns) to appear in every row's history for
  // customers who appear across many uploads.
  const byNumberId = new Map();
  ...
  events.forEach((event) => {
    ...
    if (!numberKey) return; // skip events not yet matched to a specific row
    ...
    if (!byNumberId.has(numberKey)) byNumberId.set(numberKey, []);
    byNumberId.get(numberKey).push(mappedEvent);
  });

  return { byNumberId, byMobile: new Map() };
}
...
function mergeReportReplyFields(numberRow, senderReport, inboundEvents) {
  ...
  // replyHistory: built exclusively from exact-matched events (matchedNumberId = this row).
  // If no exact events exist yet (write-back hasn't run) but customReply is confirmed,
  // create a single synthetic entry so the history cell is not empty.
  let replyHistory = [];
  if (customReply) {
    if (inboundList.length > 0) {
      const seenTexts = new Set();
      replyHistory = inboundList
        .filter((e) => e && e.text)
        .sort(
          (a, b) =>
        ...
        .map((e) => ({
          text: e.text,
          receivedAt: e.receivedAt || e.statusUpdatedAt || e.updatedAt || "",
          customerNumber:
            e.normalizedMobile || e.customerNumber || numberRow.cleaned,
          requestId: e.requestId || null,
          eventKey: e.eventKey || null,
          rawPayload: e.rawPayload || null,
        }))
        .filter((item) => {
          const key = `${item.receivedAt}|${item.text}`;
          if (seenTexts.has(key)) return false;
          seenTexts.add(key);
          return true;
        });
    } else {
      // Exact events not yet available: synthesise one entry from the confirmed reply.
      replyHistory = [
        {
          text: customReply,
          receivedAt: numberRow.lastReplyAt || senderReport?.lastReplyAt || "",
          customerNumber: numberRow.cleaned || "",
          requestId: null,
          eventKey: null,
          rawPayload: null,
        },
  ...
  const inboundTime =
    latestInbound?.receivedAt ||
    latestInbound?.statusUpdatedAt ||
    latestInbound?.updatedAt ||
    null;
  ...
  return {
    ...
    inboundReply: latestInbound || null,
  };
}
...
function formatSingleRowForExcel(row, index) {
  return {
    "S.No.": index + 1,
    "Date & Time": formatDateTimeForReport(row.requestedAt || row.sentAt || row.receivedAt || row.statusUpdatedAt),
    "Customer Name": getCustomerNameForReport(row),
