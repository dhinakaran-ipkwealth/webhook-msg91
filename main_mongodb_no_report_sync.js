...
const scheduleFilePath = path.join(app.getPath("userData"), "report-schedule.json");
  ...
  scheduleTimer = setTimeout(async function runAndReschedule() {
    try {
      if (cfg.mechanism === "email") {
        console.log(`Scheduled run triggered. Generating 24-hour RM and Admin reports (from configured ${cfg.time} yesterday to ${cfg.time} today)...`);
        ...
        await sendAdminReportEmailDirect(fullRangeResult.filePath, start.toISOString(), end.toISOString());
...
async function sendAdminReportEmailDirect(filePath, startIso, endIso) {
  if (String(process.env.DISABLE_EMAIL_DELIVERY || "").toLowerCase() === "true") {
    console.log(`Email delivery disabled by DISABLE_EMAIL_DELIVERY; would have delivered admin report: ${filePath}`);
  ...
  const to = (process.env.ADMIN_EMAILS_TO || "prabhukumarasamy@ipkwealth.com").split(",").map(s=>s.trim()).filter(Boolean);
  const cc = [process.env.ADMIN_EMAILS_CC_SALES1, process.env.ADMIN_EMAILS_CC_SALES2].map(s=>s?.trim()).filter(Boolean);
  const bcc = (process.env.ADMIN_EMAILS_BCC || "dhinakaran@ipkwealth.com,vijaytp@ipkwealth.com").split(",").map(s=>s.trim()).filter(Boolean);

  if (!to.length) throw new Error("No recipient addresses configured for admin emails.");

  const startLabel = startIso ? new Date(startIso).toLocaleString() : "";
  const endLabel = endIso ? new Date(endIso).toLocaleString() : "";
  const rangeStr = startLabel && endLabel ? `${startLabel} to ${endLabel}` : "Daily Webhook Feed";

  const mailOptions = {
    from: process.env.SMTP_FROM || (process.env.SMTP_USER || "software@ipkwealth.com"),
    to: to.join(","),
...
async function sendRmGroupedReportsDirect(filters = {}) {
  ...
  if (!rms.length) {
    console.log("No RMs configured in msg91.config.json for grouped report delivery.");
    return { count: 0, sent: 0 };
  }
  ...
  for (const rm of rms) {
    ...
    const rmRows = allRows.filter((row) => {
      const rowSender = String(row.integratedNumber || row.integrated_number || "").trim();
      return rowSender === rmNumber || rowSender === `client-${rmNumber}`;
    });

    if (!rmRows.length) {
      console.log(`No transactions found for RM ${rm.label} (${rmNumber}). Skipping email.`);
      continue;
    }
    ...
    if (!rmEmail) {
      console.warn(`No email address configured or resolved for RM ${rm.label} (${rmNumber}). Skipping delivery.`);
      continue;
    }
    ...
    // Send email to RM
    if (String(process.env.DISABLE_EMAIL_DELIVERY || "").toLowerCase() === "true") {
      console.log(`[DRY RUN] Would have sent RM report to ${rmEmail} with file ${filePath}`);
      rmSentCount++;
      continue;
    }
    ...
    await transporter.sendMail(mailOptions);
    console.log(`RM report successfully sent to ${rmEmail} for sender ${rmNumber}`);
    rmSentCount++;
  }
...
function getMongoConfig() {
  ...
  const uri =
    ...
    "";
  const dbName = envValues.MONGODB_DB_NAME || process.env.MONGODB_DB_NAME || "";
  return { uri, dbName };
}
...
async function initMongo() {
  ...
  // Prefer using the shared mongoGateway if available
  try {
    const mongoGateway = require('./src/gateways/mongoGateway');
    const initResult = await mongoGateway.init(uri, dbName);
    if (initResult && initResult.connected) {
      try { await mongoGateway.ensureIndexes(); } catch (e) { /* ignore */ }
      mongoDb = mongoGateway.getDb();
      return;
    }
  } catch (e) {
  ...
  try {
    ...
    await safeCreateIndex(mongoDb.collection('whatsapp_uploads'), { uploadId: 1 }, { unique: true });
    await safeCreateIndex(mongoDb.collection('whatsapp_numbers'), { uploadId: 1, numberId: 1 }, { unique: true });
    await safeCreateIndex(mongoDb.collection('whatsapp_numbers'), { uploadId: 1, id: 1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_numbers'), { uploadId: 1, cleaned: 1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_numbers'), { updatedAt: -1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_webhook_events'), { receivedAt: -1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_webhook_events'), { matchedUploadId: 1, receivedAt: -1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_webhook_events'), { uploadId: 1, receivedAt: -1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_webhook_events'), { source: 1, receivedAt: -1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_webhook_events'), { source: 1, sourceEventId: 1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_webhook_events'), { normalizedMobile: 1, receivedAt: -1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_webhook_events'), { eventType: 1, normalizedStatus: 1, receivedAt: -1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_webhook_events'), { eventKey: 1 }, { unique: true });
    await safeCreateIndex(mongoDb.collection('whatsapp_webhook_events'), { stableKey: 1 }, { sparse: true });
    await safeCreateIndex(mongoDb.collection('whatsapp_webhook_events'), { source: 1, stableKey: 1 }, { unique: true, partialFilterExpression: { stableKey: { $type: 'string' } } });
    await safeCreateIndex(mongoDb.collection('whatsapp_webhook_events'), { modifiedAt: -1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_sender_reports'), { senderNumber: 1, templateName: 1, uploadId: 1, numberId: 1 }, { unique: true });
    await safeCreateIndex(mongoDb.collection('whatsapp_sender_reports'), { uploadId: 1, numberId: 1 });
    await safeCreateIndex(mongoDb.collection('whatsapp_sender_reports'), { senderNumber: 1, teamLabel: 1, updatedAt: -1 });
  } catch (error) {
    console.warn('MongoDB connection skipped:', error.message || error);
    mongoDb = null;
    if (mongoClient) {
...
function createWindow() {
  ...
  mainWindow.loadFile(path.join(__dirname, 'legacy', 'index_modern_saas_complete.html'));
}
...
async function requireMongoDb() {
  if (!mongoDb || isDbClosing) {
    // try to reuse shared mongoGateway db instance if present
    try {
      const mongoGateway = require('./src/gateways/mongoGateway');
...
async function listNumbersByUpload(uploadId, options = {}) {
  ...
  if (options.validOnly) query.valid = 1;
  if (options.deliveryStatus) query.deliveryStatus = options.deliveryStatus;
  const rows = await mongoGateway.findNumbersByUpload(Number(uploadId), options.projection ? options.projection : null, options.desc ? { id: -1 } : { id: 1 });
...
async function getNumberById(numberId, projection = null) {
  await requireMongoDb();
  const mongoGateway = require('./src/gateways/mongoGateway');
  const rows = await mongoGateway.findNumbersByIds([Number(numberId)]);
...
async function getUploadById(uploadId) {
  await requireMongoDb();
  const mongoGateway = require('./src/gateways/mongoGateway');
  const uploads = await mongoGateway.findUploadsByDateRange('1970-01-01T00:00:00.000Z', new Date().toISOString());
  const row = uploads.find(u => Number(u.id) === Number(uploadId)) || null;
  return row ? normalizeMongoDoc(row) : null;
}
...
async function findLatestNumberByMobile(mobile, uploadId = null) {
  ...
  const mongoGateway = require('./src/gateways/mongoGateway');
  const rows = await mongoGateway.findNumbersByMobiles([String(mobile || "")]);
  const row = rows && rows.length ? rows[0] : null;
...
async function findNumberByMessageKey(normalized, context = {}) {
  ...
  const mongoGateway = require('./src/gateways/mongoGateway');
  const db = mongoGateway.getDb();
  const candidates = await db.collection('whatsapp_numbers').find(query).sort({ id: 1 }).toArray();
...
async function findNumberByWebhookContent(normalized, context = {}) {
  ...
  return best && bestScore >= 2 ? { id: best.id, uploadId: best.uploadId } : null;
}
...
async function findLatestNumberForWebhook(normalized, context = {}) {
  ...
  return findLatestNumberByMobile(normalized.normalizedMobile, context.uploadId || null);
}

async function getNumberIdsByUploadAndMobile(uploadId, cleaned) {
  await requireMongoDb();
  const mongoGateway = require('./src/gateways/mongoGateway');
  const rows = await mongoGateway.findNumbersByUpload(Number(uploadId), { id: 1, cleaned: 1 }, null);
  return rows.filter(r => String(r.cleaned) === String(cleaned)).map(normalizeMongoDoc);
}
...
async function insertUploadDoc(upload) {
  ...
  const mongoGateway = require('./src/gateways/mongoGateway');
  await mongoGateway.updateUpload({ uploadId: id }, { $set: doc }, { upsert: true });
...
async function insertNumberDoc(number) {
  ...
  const mongoGateway = require('./src/gateways/mongoGateway');
  await mongoGateway.updateNumber({ numberId: id }, { $set: doc }, { upsert: true });
  return makeRunResult(id, 1);
}
...
async function updateUploadFields(uploadId, fields) {
  await requireMongoDb();
  const mongoGateway = require('./src/gateways/mongoGateway');
  await mongoGateway.updateUpload({ id: Number(uploadId) }, { $set: { ...fields, updatedAt: new Date().toISOString() } });
...
async function updateNumberFields(numberId, fields) {
  await requireMongoDb();
  const mongoGateway = require('./src/gateways/mongoGateway');
  await mongoGateway.updateNumber({ id: Number(numberId) }, { $set: { ...compactObject(fields), updatedAt: new Date().toISOString() } });
  return makeRunResult(null, 1);
}

async function updateNumberFieldsWithInc(numberId, fields, inc = null) {
  await requireMongoDb();
  const update = { $set: { ...compactObject(fields), updatedAt: new Date().toISOString() } };
...
async function listUploadsWithCountsForDateRange(startIso, endIso) {
  await requireMongoDb();
  const mongoGateway = require('./src/gateways/mongoGateway');
  const uploadRows = await mongoGateway.findUploadsByDateRange(startIso, endIso);

  const output = [];
  for (const upload of uploadRows) {
    const rows = await mongoGateway.findNumbersByUpload(upload.id);
...
async function listCustomReportRowsFromMongo(filters = {}) {
  await requireMongoDb();
  try {
    const gen = require('./src/usecases/generateReport');
    return await gen.getCustomReportRows(filters);
  } catch (e) {
    // If the usecase cannot be loaded, fall back to in-file implementation (shouldn't happen)
    throw e;
  }
}
...
function findMobileField(headers) {
  ...
  const mobileHeader = normalized.find((header) =>
    /mobile|phone|number/.test(header),
  );
...
async function storeUpload(filePath, fileName, rawRows) {
  const rows = rawRows.map((rowData) => {
...
function normalizeMsg91WebhookItem(item) {
  ...
  return {
    eventType,
    uploadId: item.upload_id || item.uploadId || item.metadata?.upload_id || null,
    normalizedMobile,
    normalizedStatus:
...
async function processWebhookReport(body, context = {}) {
  ...
  for (const rawItem of payload) {
    ...
    const storedEvent = await storeWebhookEvent(item, msg91Normalized, finalMatch, context);
...
function reportItemMatchesUploadContext(normalized, upload) {
  const uploadSender = formatPhoneForCall(upload.senderId || upload.senderNumber || "");
  ...
  const uploadTemplate = String(upload.templateName || upload.templateLabel || "").trim().toLowerCase();
  const itemTemplate = String(normalized.templateName || "").trim().toLowerCase();
  if (uploadTemplate && itemTemplate && uploadTemplate !== itemTemplate) {
    const uploadTemplateSlug = uploadTemplate.replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
    const itemTemplateSlug = itemTemplate.replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
...
async function applyReportItemsToUpload(upload, rawItems, sourceLabel = "MSG91 report") {
  const items = Array.isArray(rawItems) ? rawItems : [];
  const uploadRows = await mongoDb
    .collection("whatsapp_numbers")
    .find(
      { uploadId: Number(upload.id) },
      { projection: { id: 1, cleaned: 1, original: 1, responseId: 1, messageId: 1 } },
    )
    .toArray();
  ...
  for (const item of items) {
    ...
    const responseId = normalized.responseId ? String(normalized.responseId) : "";
    const candidates = uploadRows.filter((row) => {
      if (responseId && (String(row.responseId || "") === responseId || String(row.messageId || "") === responseId)) {
        return true;
      }
      if (!responseId && cleaned && mobileCounts.get(cleaned) === 1) {
        return formatPhoneForCall(row.cleaned || row.original || "") === cleaned;
      }
      return false;
    });
...
async function requestLogsReportForUpload(upload) {
  const startDate = toMsg91Date(upload.createdAt || upload.triggeredAt || new Date());
  ...
  const result = await applyReportItemsToUpload(upload, items, "MSG91 WhatsApp logs");
...
async function requestReportForUpload(upload, options = {}) {
  ...
  if (!upload.apiMessageId || !upload.apiKey) {
    const message = "This upload has no MSG91 message id/auth key yet. Send the campaign before refreshing server report data.";
  ...
  try {
    ...
    const result = await applyReportItemsToUpload(upload, rawItems, "MSG91 message report");
    ...
    if (getHttpStatus(error) === 404 && upload.apiKey) {
      try {
        ...
        if (throwOnError) {
          const message = logsError.response?.data?.message || logsError.message || "MSG91 logs refresh failed";
          throw new Error(message);
        }
        return {
          updated: 0,
          skipped: 0,
          error: logsError.response?.data?.message || logsError.message || "MSG91 logs refresh failed",
        };
      }
    }
    await handleReportPollFailure(upload, error);
    if (throwOnError) {
      const message = error.response?.data?.message || error.message || "MSG91 report refresh failed";
      throw new Error(message);
    }
    return {
      updated: 0,
      skipped: 0,
      error: error.response?.data?.message || error.message || "MSG91 report refresh failed",
    };
  }
}
...
function startWebhookServer() {
  ...
  const handleWebhook = async (req, res, context = {}) => {
    try {
      // Prefer the new centralized use-case which handles dedupe, upsert, and applying replies/reports.
      try {
        const { storeWebhook } = require('./src/usecases/storeWebhook');
        await storeWebhook(req.body, context);
      } catch (e) {
        // fallback to legacy processor if storeWebhook is not available
...
async function getSenderReportMapForUpload(uploadId) {
  await requireMongoDb();
  const mongoGateway = require('./src/gateways/mongoGateway');
  const uploadNumberRows = await mongoGateway.findNumbersByUpload(Number(uploadId), { id: 1, numberId: 1, cleaned: 1, original: 1 });
  const mobileCounts = new Map();
  ...
  const senderReports = await mongoGateway.findSenderReports({ uploadId: Number(uploadId) }, { sort: { updatedAt: -1, sentAt: -1, _id: -1 } });
  ...
  senderReports.forEach((report) => {
    ...
    const mobileKey = formatPhoneForCall(report.mobile || report.cleaned || report.customerNumber || "");

    if (numberKey && !byNumberId.has(numberKey)) byNumberId.set(numberKey, report);
    if (mobileKey && mobileCounts.get(mobileKey) === 1 && !byMobile.has(mobileKey)) byMobile.set(mobileKey, report);
  });
...
async function getInboundReplyMapForUpload(uploadId) {
  ...
  const mobileRegexConditions = mobiles.flatMap((mobile) => {
    ...
    return [
      ...
      ...(local && local !== mobile
        ? [{ messages: { $regex: local } }, { "rawPayload.messages": { $regex: local } }]
        : []),
    ];
  });

  const events = await mongoGateway.findWebhookEvents({
    eventType: "inbound",
    $or: [
  ...
  }, { sort: { receivedAt: -1, updatedAt: -1, _id: -1 }, limit: 2000 });
  ...
  events.forEach((event) => {
    const mobile = formatPhoneForCall(
      event.normalizedMobile ||
        ...
        extractMobileFromWebhookMessages(event.rawPayload?.messages || event.messages) ||
        "",
    );
    const numberKey = Number(event.matchedNumberId || event.numberId || event.messageNumberId || 0);
    const text =
      event.text ||
      event.customReply ||
      event.button?.text ||
    ...
    if (numberKey && !byNumberId.has(numberKey)) byNumberId.set(numberKey, mappedEvent);
    if (mobile && mobileCounts.get(mobile) === 1 && !byMobile.has(mobile)) byMobile.set(mobile, mappedEvent);
  });
  return { byNumberId, byMobile };
}
...
function mergeReportReplyFields(numberRow, senderReport, inboundEvent) {
  ...
  const inboundTime = inboundEvent?.receivedAt || inboundEvent?.statusUpdatedAt || inboundEvent?.updatedAt || null;
  const customReply =
    numberRow.customReply ||
    senderReport?.customReply ||
    inboundText ||
    "";
  const replyHistory = Array.isArray(existingHistory) ? [...existingHistory] : [];
  if (Array.isArray(senderHistory)) {
    senderHistory.forEach((item) => replyHistory.push(item));
  }
  if (inboundText && !replyHistory.some((item) => String(item?.text || "") === String(inboundText))) {
    replyHistory.push({
      text: inboundText,
      receivedAt: inboundTime || new Date().toISOString(),
      customerNumber: inboundEvent.normalizedMobile || inboundEvent.customerNumber || numberRow.cleaned,
      rawPayload: inboundEvent.rawPayload || inboundEvent,
    });
  }
  const numberUpdatedAt = new Date(numberRow.lastUpdated || numberRow.updatedAt || numberRow.sentAt || 0).getTime() || 0;
  const senderUpdatedAt = new Date(senderReport?.updatedAt || senderReport?.lastUpdated || senderReport?.sentAt || 0).getTime() || 0;
  const statusSource = senderReport && senderUpdatedAt >= numberUpdatedAt ? senderReport : numberRow;

  return {
    customReply,
    replyHistory: replyHistory.slice(-50),
    lastReplyAt: numberRow.lastReplyAt || senderReport?.lastReplyAt || inboundTime || "",
    currentStatus: customReply ? "replied" : (statusSource.currentStatus || numberRow.currentStatus),
    ...
    responseDetails: statusSource.responseDetails || parseJsonField(numberRow.responseDetails, null),
...
ipcMain.handle("fetch-report", async (event, uploadId) => {
  ...
  return rows.map((row) => {
    ...
    const mobile = formatPhoneForCall(row.cleaned || row.original || "");
    const senderReport = senderReportMap.byNumberId.get(numberId) || senderReportMap.byMobile.get(mobile) || null;
    const inboundEvent = inboundReplyMap.byNumberId.get(numberId) || inboundReplyMap.byMobile.get(mobile) || null;
...
ipcMain.handle("fetch-sender-stats", async (event, filters = {}) => {
  ...
  if (filters.todayOnly) {
    ...
    match.sentAt = {};
    if (filters.startDateTime) match.sentAt.$gte = new Date(filters.startDateTime).toISOString();
    if (filters.endDateTime) match.sentAt.$lt = new Date(filters.endDateTime).toISOString();
  }
    ...
    { $match: match },
    {
      $group: {
        ...
        responses: {
          $sum: {
            $cond: [
              { $or: [ { $ne: ["$customReply", null] }, { $ne: ["$lastReplyAt", null] } ] },
              1,
              0,
            ],
          },
  ...
  return {
    ...
    templatesTriggered: Array.isArray(row.templates) ? row.templates.filter(Boolean).length : 0,
...
ipcMain.handle("schedule-set", async (event, cfg = {}) => {
  ...
  scheduleConfig = { enabled: Boolean(cfg.enabled), time: cfg.time || "10:00", mechanism: cfg.mechanism || "email" };
...
ipcMain.handle("schedule-run-now", async () => {
  try {
    const cfg = scheduleConfig || loadScheduleConfig() || { enabled: true, time: "10:00", mechanism: "email" };
    if (cfg && cfg.mechanism === "email") {
      console.log("Manual trigger for schedule-run-now starting (Email flow with 24-hour RM/Admin split)...");
      const [hh, mm] = (cfg.time || "10:00").split(":").map((v) => Number(v));
      const end = new Date();
      end.setHours(hh, mm, 0, 0);
      const start = new Date(end);
      ...
      await sendAdminReportEmailDirect(fullDayResult.filePath, start.toISOString(), end.toISOString());
      ...
      if (cfg && cfg.mechanism === "sftp") await deliverFileBySftp(res.filePath);
      return res;
    }
  } catch (err) {
    throw err;
...
function normalizeReportStatus(row) {
  const status = String(row.deliveryStatus || row.currentStatus || "").toLowerCase();
  if (status.includes("delivered") || status.includes("read")) return "delivered";
  if (status.includes("failed") || status.includes("undelivered") || status === "invalid") return "failed";
  if (status.includes("sent") || status.includes("submitted")) return "sent";
  if (status.includes("reporting") || status.includes("pending")) return "reporting";
...
function reportRowMatchesDateRange(row, filters = {}) {
  ...
  const value = row.receivedAt || row.statusUpdatedAt || row.requestedAt || row.updatedAt || "";
...
function getReplyTextFromHistory(replyHistory) {
  ...
  const latest = [...history]
    ...
    .sort((a, b) => new Date(b.receivedAt || b.updatedAt || 0).getTime() - new Date(a.receivedAt || a.updatedAt || 0).getTime())[0];
  return latest?.text || latest?.customReply || "";
}

function normalizeReplyHistoryItems(replyHistory, fallbackText = "", fallbackTime = "", fallbackPayload = null) {
  const seen = new Set();
  const items = [];
  const history = Array.isArray(replyHistory) ? replyHistory : [];
  ...
  history.forEach((reply) => {
    ...
    if (!text) return;
    const receivedAt = reply.receivedAt || reply.lastReplyAt || reply.updatedAt || fallbackTime || "";
  ...
  return items.sort((a, b) => new Date(b.receivedAt || 0).getTime() - new Date(a.receivedAt || 0).getTime());
}
...
async function listSelectedUploadReportRows(filters = {}) {
  ...
  rows.forEach((row) => {
    ...
    const inboundEvent = inboundReplyMap.byNumberId.get(numberId) || inboundReplyMap.byMobile.get(mobile) || null;
    const mergedReply = mergeReportReplyFields(row, senderReport, inboundEvent);
    const latestReplyText = mergedReply.customReply || getReplyTextFromHistory(mergedReply.replyHistory);
    const outboundPayload = mergedReply.responseDetails || senderReport?.responseDetails || {};
    const baseRow = {
      ...
      customerName: "",
      integratedNumber: upload.senderId || upload.senderNumber || senderReport?.senderNumber || "",
      integrated_number: upload.senderId || upload.senderNumber || senderReport?.senderNumber || "",
      templateName: upload.templateName || senderReport?.templateName || "",
      campaignName: "",
      receivedAt: mergedReply.senderReport?.sentAt || upload.sentAt || row.lastUpdated || upload.createdAt || "",
      requestedAt: mergedReply.senderReport?.sentAt || upload.sentAt || upload.createdAt || "",
      ...
      numberDeliveryStatus: mergedReply.deliveryStatus || row.deliveryStatus || "",
    ...
    replyItems.forEach((reply, replyIndex) => {
      mappedRows.push({
        ...
        receivedAt: reply.receivedAt || baseRow.lastReplyAt || baseRow.receivedAt,
        requestedAt: reply.receivedAt || baseRow.lastReplyAt || baseRow.requestedAt,
  ...
  return mappedRows.filter((row) => {
    if (filters.filteredNumberId && filters.filteredNumberId !== "all") {
      ...
      const rowSender = normalizeSenderFilterValue(row.integratedNumber || row.integrated_number || "");
      if (selectedSender && rowSender && selectedSender !== rowSender) return false;
    }

    if (filters.templateName && filters.templateName !== "all") {
      const candidates = getTemplateFilterCandidates(filters.templateName);
      const rowTemplates = getTemplateFilterCandidates(row.templateName || row.uploadTemplateLabel || "");
      if (!rowTemplates.some((value) => candidates.includes(value))) return false;
    }
    ...
    return reportRowMatchesSearch(row, filters.search) && reportRowMatchesDateRange(row, filters);
  }).sort((a, b) => {
  ...
  if (isMultiSheet) {
    ...
    rms.forEach((rm) => {
      ...
      const rmRows = rows.filter((row) => {
        const rowSender = String(row.integratedNumber || row.integrated_number || "").trim();
        const matches = rowSender === rmNumber || rowSender === `client-${rmNumber}`;
        if (matches) {
      ...
      if (rmRows.length > 0) {
        const formattedRows = rmRows.map((r, index) => formatSingleRowForExcel(r, index));
        const worksheet = XLSX.utils.json_to_sheet(formattedRows);
        XLSX.utils.book_append_sheet(workbook, worksheet, getSafeSheetName(rm.label));
      }
    });

    // 2. Collate any unmatched transactions and append to an "Other Event Logs" sheet
    const unmatchedRows = rows.filter((row) => {
      return !matchedRowIds.has(row.id || row._id || String(row.integratedNumber || row.integrated_number || ""));
    });

    if (unmatchedRows.length > 0) {
      const formattedRows = unmatchedRows.map((r, index) => formatSingleRowForExcel(r, index));
      const worksheet = XLSX.utils.json_to_sheet(formattedRows);
    ...
    if (workbook.SheetNames.length === 0) {
      const formattedRows = rows.map((r, index) => formatSingleRowForExcel(r, index));
      const worksheet = XLSX.utils.json_to_sheet(formattedRows);
  ...
  try {
    ...
    try {
      ...
      if (!scheduleConfig) {
        ...
        saveScheduleConfig(scheduleConfig);
        console.log("Initialized default 10:00 AM daily email schedule on first-time launch.");
      }
      if (scheduleConfig && scheduleConfig.enabled) scheduleNextRun(scheduleConfig);
    } catch (err) {
